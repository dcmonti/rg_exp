...
pub fn index_paths(gfa_file: &str) -> PathIndex {
    ...
    for p in &gfa.paths {
        let path_id = index.paths.len();
        index.paths_to_ids.insert(path_id, p.path_name.clone());

        let new_nodes: Vec<(Vec<u8>, bool, usize)> = p.iter()
            .enumerate()
            .map(|(pos, (node_id, orientation))| {
                (node_id.to_vec(), orientation == gfa::gfa::Orientation::Forward, pos)
            })
            .collect();

        for (node_id, _, pos) in &new_nodes {
            index.update_path_occurrence(node_id, path_id, *pos);
        }

        let nodes = new_nodes.into_iter().map(|(id, dir, _)| (id, dir)).collect();
        index.paths.insert(path_id, PathInfo::new(p.path_name.clone(), nodes));
    }
